...
// Constants
var halfOrthographicSizeWithAspectRatio = (new vec2(script.orthoCamera.size * script.orthoCamera.aspect, script.orthoCamera.size)).uniformScale(0.5);
...
function centerControlDot() {
    ...
    if (distance > 0.005) {
        var next = vec2.lerp(current, target, 0.2);
...
function setLookAtEnabled(status) {
    var lookAtComponent = script.sceneCamera.getSceneObject().getComponent("Component.LookAtComponent");
    lookAtComponent.enabled = status;
}

function setGyroEnabled(status) {
    var deviceTracking = script.sceneCamera.getSceneObject().getComponent("Component.DeviceTracking");
